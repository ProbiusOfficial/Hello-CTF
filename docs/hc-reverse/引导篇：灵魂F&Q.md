...
你说得对，但逆向工程一般指软件逆向工程，即对 **目标文件** 进行分析（如可执行文件及其链接库），通过反汇编工具查看文件机器码对应的汇编块，研究程序的行为和算法，然后以此为依据，通过一系列操作得出出题人想要隐藏的 flag。

当然，这只是通俗易懂的针对 CTF 比赛的解释，实战中的逆向工程，会涉及到更多方面，解释也会更加复杂，这里不再赘述。

![p1-1](./Images/p1-1.png)

💡 Tips：建议尽你所能去了解一下什么是 **目标文件**，本板块中不会对目标文件进行很详细的解释。
