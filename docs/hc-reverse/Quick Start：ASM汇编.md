...
以上代码将 `ebx` 的值与 `0` 对比，如果相等，则会跳转到 `label` 处，条件跳转指令 `je` 代表相等则跳转，还有其他与之条件不一样的条件跳转指令，一般条件跳转指令是与 `cmp`、`test` 指令混在一起用的。以上代码也可以用 `test` 来写：
...
在汇编中，我们有时候需要知道参数被保存在哪个寄存器里，这里我列举一般情况：
