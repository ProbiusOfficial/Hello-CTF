...
-   :fontawesome-solid-blog:{ .lg .middle } __最近更新__
    ...
    记录一下今年 2026 决赛过程题目描述（1）  「影」核心系统「根」需要在一些开启了特殊特性的机器上才能部署成功，逆向找到成功部署条件！成功部署「影」核心系统，即成功运行shadow_panel.exe，控制台程序成功运行进入至输入终止密码的终端。(满分0.5分)（2）  「根」使用特殊方法，对操作系统底层进行了攻击，并借此将关键核心代码隐藏了起来，分析其完整实现流程。（满分2.5分）（3）...
