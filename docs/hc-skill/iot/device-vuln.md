...
- **传感器欺骗**:物理层伪造传感器读数(4-20mA 信号发生器、温度/湿度探头替换、GPS 欺骗)→ 让设备基于假数据动作;网络层重放传感器报文(→ [ICS-物理安全](../ics/physical.md) 设备篡改)。
- **侧信道攻击**:设备功耗/电磁/时序泄露密钥(→ [ICS-物理安全](../ics/physical.md) 旁路攻击);嵌入式加密芯片的 DPA。
- **固件更新劫持**:OTA 通道中间人替换升级包(未验签 → 任意固件,→ [固件分析](firmware.md) 固件签名验证);降级攻击刷回带漏洞旧版;更新服务器入侵(→ [供应链安全](../ics/supply-chain.md))。
- **设备克隆**:Flash 整片读出(→ [固件分析](firmware.md) 固件提取)后写入新芯片——凭证/密钥随镜像被克隆;云端"设备指纹"(MAC/序列号)未绑定安全元件时克隆即冒名。
- **设备劫持**:拿下设备后反向控制其云会话(窃取的 token 注入)、僵尸网络化(Mirai 模式:telnet 弱口令 → 蠕虫植入)、DNS/网关重定向让设备流量过攻击者。
- **数据泄露**:设备本地敏感数据(缓存录像、用户凭证、wifi 密码)经调试口/未授权服务导出;云端 API 越权拉取他人设备数据(IDOR,→ [WEB-逻辑漏洞](../web/logic-vuln.md))。
- **物理访问攻击**:拿到硬件本体后的攻击总称——调试口、芯片读取、篡改(→ [ICS-物理安全](../ics/physical.md))。
- **配置错误**:管理接口暴露公网、UPnP 全开、云对接密钥写在公开固件、防火墙策略放行调试端口(→ [网络安全](network.md) 服务枚举)。
- **硬件调试接口**:UART/JTAG/SWD 的暴露与利用(→ [固件分析](firmware.md) 提取节与 [ICS-物理安全](../ics/physical.md) 硬件调试接口)。
